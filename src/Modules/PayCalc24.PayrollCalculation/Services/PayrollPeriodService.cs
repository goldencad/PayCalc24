    ...
    IPayrollSnapshotResolver resolver) : IPayrollPeriodService, IPayrollSnapshotQueryService
{
    ...
    public IReadOnlyList<Contracts.PayrollFunds.SnapshotPayrollFundVersion> GetFundVersions(CompanyId companyId,PayrollCalculationSnapshotId snapshotId)=>GetSnapshotById(companyId,snapshotId).PolicyConfiguration.FundVersions??[];
...
internal static class SnapshotHasher
{
    ...
    internal static Hashes Hash(CompanyId company,PayrollPeriodId period,int revision,DateOnly businessDate,PayrollSnapshotCandidate candidate)
    {
        ...
        var cfg=Digest(string.Join('\n',new[]{string.Join(';',candidate.PolicyConfiguration.CompensationVersions.OrderBy(x=>x.CompensationSchemeId.Value).Select(x=>$"C:{x.CompensationSchemeId.Value:D}:{x.SchemeVersion}:{string.Join(',',x.Components.OrderBy(y=>y.Sequence).ThenBy(y=>y.PayComponentId.Value).Select(y=>$"{y.PayComponentId.Value:D}:{y.Version}:{y.Sequence}:{y.CalculationMethod}:{y.FormulaDefinitionId?.Value:D}:{y.ComponentCode}:{y.Required}:{y.SourceReference}:{y.ExpectedDataType}:{string.Join('+',(y.DependsOn??[]).OrderBy(z=>z.Value).Select(z=>z.Value.ToString("D")))}"))}")),string.Join(';',candidate.PolicyConfiguration.FormulaVersions.OrderBy(x=>x.FormulaDefinitionId.Value).Select(x=>$"F:{x.FormulaDefinitionId.Value:D}:{x.FormulaVersionId.Value:D}:{x.Revision}:{x.Checksum}:{x.Expression}")),string.Join(';',candidate.PolicyConfiguration.ParameterVersions.OrderBy(x=>x.Code,StringComparer.Ordinal).Select(x=>$"P:{x.ParameterSetVersionId.Value:D}:{x.Revision}")),string.Join(';',candidate.PolicyConfiguration.LookupVersions.OrderBy(x=>x.Code,StringComparer.Ordinal).Select(x=>$"L:{x.LookupTableVersionId.Value:D}:{x.Revision}:{string.Join(',',x.Rows.OrderBy(y=>y.Sequence).Select(y=>y.Id.ToString("D")))}")),string.Join(';',candidate.PolicyConfiguration.RuleSetVersions.OrderBy(x=>x.Code,StringComparer.Ordinal).Select(x=>$"R:{x.RuleSetVersionId.Value:D}:{x.Revision}:{string.Join(',',x.Rules.OrderBy(y=>y.Priority).ThenBy(y=>y.Id).Select(y=>y.Id.ToString("D")))}")),string.Join(';',(candidate.PolicyConfiguration.FundVersions??[]).OrderBy(x=>x.FundVersionId.Value).Select(x=>$"U:{x.FundVersionId.Value:D}:{x.Revision}:{x.Code}:{x.Source.Type}:{x.Source.FixedAmount?.ToString(CultureInfo.InvariantCulture)}:{x.Source.InputCode}:{x.Source.FormulaVersionId?.Value:D}:{x.Policy.Method}:{x.Policy.DecimalScale}"))}));
